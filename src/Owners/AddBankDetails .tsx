...
const bankdetilsAdd = async(e:React.FormEvent<HTMLFormElement>) =>{
  e.preventDefault()
  try{
    const response = await AxiosApi.post(`/api/owner/bank/${owner._id}`,data)
    ...
    <div className="max-w-md mx-auto p-6 bg-white rounded-md shadow-md mt-4">
      ...
      <form onSubmit={bankdetilsAdd}>
        ...
        <div className="mb-4">
          ...
          <input type="text" id="accountNumber" name="AccountNumber" className="mt-1 p-2 border rounded-md w-full" required onChange={handlechange} />
        </div>
        <div className="mb-4">
