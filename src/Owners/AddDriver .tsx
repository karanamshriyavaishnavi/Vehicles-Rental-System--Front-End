...
const Doctor = async(e:React.FormEvent<HTMLFormElement>) =>{
  ...
  try{
    const response = await AxiosApi.put(`/api/owner/status/${product._id}`,formdata)
    ...
    <div className="max-w-md mx-auto p-6 bg-white rounded-md shadow-md mt-4">
      ...
      <form onSubmit={Doctor}>
        <div className="mb-4">
          ...
          <input type="text" id="driverName" name="driverName" className="mt-1 p-2 border rounded-md w-full" required  onChange={handlechage}/>
        </div>
        <div className="mb-4">
          <label htmlFor="age" className="block text-sm font-medium text-gray-700">Age</label>
          <input type="number" id="age" name="age" className="mt-1 p-2 border rounded-md w-full" required onChange={handlechage} />
        </div>
        <div className="mb-4">
          <label htmlFor="experience" className="block text-sm font-medium text-gray-700">Experience (in years)</label>
          <input type="number" id="experience" name="experience" className="mt-1 p-2 border rounded-md w-full" required onChange={handlechage} />
        </div>
        <div className="mb-4">
          <label htmlFor="license" className="block text-sm font-medium text-gray-700">License</label>
          <input type="file" id="license" name="license" className="mt-1 p-2 border rounded-md w-full" required onChange={handlelisence} />
        </div>
