...
const Viewvehicals = () => {
  ...
  const gettingvehicals = async () => {
    try {
      const response = await AxiosApi.get(`/api/owner/vehical/${owner._id}`)
  ...
  const deleteVehical = async (id: string) => {
    try {
      const response = await AxiosApi.delete(`/api/owner/vehical/${id}`)
                ...
                <div className="bg-white shadow-md border border-gray-200 rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700">
                  <a href="#">
                    <img
                      ...
                      alt={item?.vehicalName || "Vehicle Image"}
                    />
